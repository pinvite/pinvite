...
async function extractFirebaseUserIdToken(
  ...
  if (
    ...
    // No cookie
    throw new Error(
      'Firebase user ID token not found in Authorization header nor in the cookie.'
    )
  }
}

async function allocateInvitationInfo(firebaseUserId: string): Promise<string> {
...
async function retrieveTwitterUserInfo(
  firebaseUserId: string
...
async function tweet(
  ...
  return tweetData.post('statuses/update', {
    status: invitationInfo.title + ' #pinvite\n' + invitationInfo.pageURL,
  })
}
  ...
  async (request: express.Request, response: express.Response) => {
    ...
    try {
      ...
      const decodedIdToken = await admin
        .auth()
        .verifyIdToken(firebaseUserIdToken)
      ...
      response
        .status(403)
        .send('Unauthorized: user not allowed to perform this action')
      return
    }
    ...
    if (!isInvitationRequest(invitationRequest)) {
      ...
      response
        .status(400)
        .send('Bad Request: properties are missing or wrong in the body')
      return
    } else {
      ...
      try {
        ...
        const invitationInfo = toInvitationInfo(
          invitationRequest,
          twitterUserInfo.userId,
          firebaseUserId,
          invitationId
        )
...
userApp.get(
  ...
  async (request, response) => {
    try {
      const invitationInfo = await retrieveInvitationInfo(
